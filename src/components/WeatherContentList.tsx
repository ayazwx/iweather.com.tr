...
const WeatherContentList: React.FC<WeatherContentListProps> = ({ data }) => {
    return (
        <div className=" w-full shadow sm:px-4  dark:bg-gray-900 max-w-full h-[300px] px-4 rounded-lg overflow-hidden relative min-w-[">
            {formatWeatherContent(data).map((content, index) => (
                <div className="flex justify-between items-center h-full px-4 max-h-[60px] border-b border-gray-700" key={index}>
